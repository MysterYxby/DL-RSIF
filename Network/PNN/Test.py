...
import os
import cv2
from model_pnn import PNN  # 假设这个模块存在并包含PNN类定义
...
class ImageTest:
    def __init__(self, device,dataset_name):
        ...
        self.scale = 2047
        self.device = device
        self.model = PNN(spectral_num = self.bands[self.dataset_name]).to(self.device)
        self.path = {
            #qb
            'qb_model_path':'pretrained-model/QB/pnn.pth',    #4bands
            ...
            'qb_fusion_dir':'output/PNN/qb',
            #gf
            'gf2_model_path':'pretrained-model/QB/pnn.pth',    #4bands
            ...
            'gf2_fusion_dir':'output/PNN/gf2',
            #wv2
            'wv2_model_path':'pretrained-model/WV2/pnn.pth',    #8bands
            ...
            'wv2_fusion_dir':'output/PNN/wv2',
            #wv3
            'wv3_model_path':'pretrained-model/WV3/pnn.pth',    #8bands
    ...
    #     """
    #     将PyTorch tensor转换为图像格式的NumPy数组。
    #     """
    ...
    #     image_np[np.where(image_np > self.scale)] = self.scale
    ...
    def process_image_and_save_tif(self, lms_data, pan_data, output_path,idx):
        ...
        处理图像并保存为TIF格式。
        """
        input_tensor = torch.cat((lms_data.unsqueeze(0), pan_data.unsqueeze(0)), 1)
        input_tensor = input_tensor.float().div(self.scale)
        with torch.no_grad():
...
if __name__ == '__main__':
    ...
    image_test = ImageTest(device, dataset_name = 'gf2')
    #模型运行
    image_test.run()